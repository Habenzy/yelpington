...
function populateRestaurantPage() {
  ...
  fetch(`http://localhost:8080/${currentRestaurant}`)
    ...
    .then((thisJson) => {
      sidebar.innerHTML += `<br><div class="sidebar-title"><b><a href="${thisJson.website}">${thisJson.name}</a><b></div><br><br>${thisJson.phone}<br><br>${thisJson.address}<br><br>${thisJson.notes}`;
      return thisJson
    })
    .then((thisJson) => {
